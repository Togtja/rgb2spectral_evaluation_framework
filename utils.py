...
def load_a_plus_oracle_model():
    ...
    spec_rec_anchors_norm = utils_sc.normc(
        loadmat("./trained_models/dictionary_a_plus_oracle.mat")["anchors"].T
    )
...
def load_a_plus_plus_model():
    ...
    # load K-SVD dictionary and normalize
    spec_rec_anchors_norm = utils_sc.normc(
        loadmat("./trained_models/dictionary_a_plus_plus.mat")["anchors"].T
    )
...
def recover_HSCNN_D(rgb, model, split_axis):
    ...
    for i in range(16):
        if split_axis == 3:
            temp = sess.run(
                net.sr, feed_dict={lr: im_lr[:, 87 * i : 87 * (i + 1), :, :]}
            )
